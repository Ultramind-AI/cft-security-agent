...
# 25. Chat-first UI and uploaded targets (T24)

The primary UI is a conversation, but chat is never a security boundary.

Invariant:

```text
ZIP / registered target
→ server-owned TargetProfile
→ user request as analysis focus
→ canonical pipeline
→ SSE snapshots
→ Evidence / FinalReport / deterministic Gate
→ grounded follow-up chat
```

Rules:

1. Browsers may upload ZIP bytes only; they never choose server repository/profile paths.
2. ZIP traversal, symlinks and archive-size abuse are rejected before Discovery.
3. Uploaded profiles are generated by deterministic Discovery and reloaded from the server-owned project root.
4. User text may focus LLM investigation, but cannot expand target identity, sandbox scope, network boundary or budgets.
5. Live chat separates agent reasoning/status from structured Evidence.
6. Follow-up answers use completed run data only and must not invent findings or verdicts.
7. Chat never bypasses Validator, sandbox enforcement, Evidence Guard or Gate.
