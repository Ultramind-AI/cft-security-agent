...
def _run_command(argv: list[str], cwd: Path, timeout: float) -> subprocess.CompletedProcess[str]:
    # Команда собирается только из доверенного TargetDefinition и фиксированных аргументов.
...
class SandboxSession:
    ...
    def __init__(
        ...
        *,
        ...
        if target.repository_path is None or target.compose_file is None:
    ...
    def _wait_until_ready(self) -> None:
        url = self.target.build_url(self.target.health_path)
        deadline = self._clock() + self.readiness_timeout
        while self._clock() < deadline:
            if self._health_probe(url, min(5.0, self.command_timeout)):
                return
            self._sleep(self.readiness_interval)
        raise SessionTimeoutError(f"Target did not become ready within {self.readiness_timeout}s: {url}")
